...
{
       0,   166,   166,   180,   181,   186,   193,   196,   201,   208,
     209,   212,   213,   214,   217,   220,   231,   244,   259,   260,
     266,   272,   273,   277,   283,   294,   311,   312,   315,   316,
...
#line 283 "scalpa.y"
                         {
        if ((yyvsp[0].typename_u)->atomic_type == VOID_A) {
            handle_error("parameter can't be of type unit");
        }
        (yyval.par_u).ident = malloc(strlen((yyvsp[-2].str_u))+1);
        MCHECK((yyval.par_u).ident);
        strncpy((yyval.par_u).ident, (yyvsp[-2].str_u), strlen((yyvsp[-2].str_u))+1);
        (yyval.par_u).ref = 0;
...
TODO table of symbol add pointer (hash table ?)
TODO program option -o <name>
TODO improve display tos function \r in display improve, print scope ?
TODO expr const or not if identifier for integer in arraytype
TODO const for array size in grammar
TODO new test for cste and expr
